...
public class DHT extends DHTBase implements IDHTResource, IDHTNode, IDHTBackground {
	...
	private NodeInfo getSucc(NodeInfo info) throws Failed {
		...
		if (localInfo.addr.equals(info.addr)) {
			...
			//TODO:
			return client.findSuccessor(info.addr, info.id);
		}
	}
...
//		routing.joinCheck();
	...
	public void join(String uri) throws Failed, Invalid {
		...
		NodeInfo info = getNodeInfo();
		NodeInfo succ;
		URI uriFormat = UriBuilder.fromUri(uri).build();
		succ = client.findSuccessor(uriFormat, info.id);
		setSucc(succ);
		stabilize();
		*/
		...
		try {
			...
			NodeInfo info = getNodeInfo();
			URI uriFormat = UriBuilder.fromUri(uri).build();
			NodeInfo succ = client.findSuccessor(uriFormat, info.id);
			setSucc(succ);
			stabilize();
		} catch (Failed e) {
			info("Join Failed");
	...
	public void listenOn(String key, EventListener listener) throws DHTBase.Failed {
		...
		NodeInfo info=getNodeInfo();
		NodeInfo succ = getSucc();
		while(succ != null&&!inInterval(id, succ.id, info.id)){
			info = succ;
			succ = getSucc(succ);
		}
		EventSource es = client.listenForBindings(info, info.id, key);
	...
	public void listenOff(String key) throws DHTBase.Failed {
		...
		client.listenOff(info, info.id, key);
	}
