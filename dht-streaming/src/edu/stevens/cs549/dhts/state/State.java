...
public class State implements IState, IRouting {
	...
	public synchronized NodeInfo closestPrecedingFinger(int id) {
		...
		for(int i = IRouting.NFINGERS -1;i>=0;i--){
			if(DHTBase.inInterval(finger[i].id,this.info.id,id))
				return finger[i];
		}
		return this.info;
	}
	...
	public void removeListener(int id, String key) {
		...
		try {
			if(outputs.get(id).get(key)!=null)
				outputs.get(id).get(key).close();
	...
	public void addListener(int id, String key, EventOutput os) {
		...
		Map<String,EventOutput> temp = new HashMap<String,EventOutput>();
		temp.put(key, os);
		outputs.put(id, temp);
