...
//	}
	...
	/*
	 * Find closestPrecedingFinger of an id.
	 */
	...
	/*
	 * Get the values under a key at the specified node.
	 */
	...
	public EventSource listenForBindings(NodeInfo node, int id, String key) throws DHTBase.Failed {
		...
		URI listenPath = ub.queryParam("id", node.id).queryParam("key", key).build();
	...
	public void listenOff(NodeInfo node, int id, String key) throws DHTBase.Failed {
		...
		UriBuilder ub = UriBuilder.fromUri(node.addr).path("listen");
		URI listenPath = ub.queryParam("id", node.id).queryParam("key", key).build();
		info("client listen off(" + listenPath + ")");

		client.target(listenPath).request(MediaType.APPLICATION_XML_TYPE)
		.header(Time.TIME_STAMP, Time.advanceTime())
		.delete();

		//		Response response = deleteRequest(listenPath);
		//		if (response == null || response.getStatus() >= 300) {
